...
class Block_Frame_Decorator extends Frame_Decorator {
  ...
  function add_frame_to_line(Frame $frame) {
    ...
    */
    ...
    if ( (float)$line->left + (float)$line->w + (float)$line->right + (float)$w > (float)$this->get_containing_block("w"))
      $this->add_line();
  ...
  function increase_line_width($w) {
    $this->_line_boxes[ $this->_cl ]->w = (float)$this->_line_boxes[ $this->_cl ]->w + (float)$w;
  }

  function maximize_line_height($val, Frame $frame) {
    $val = (float)$val;
    if ( $val > (float)$this->_line_boxes[ $this->_cl ]->h ) {
...
//       return;
    ...
    $y = (float)$this->_line_boxes[$this->_cl]->y + (float)$this->_line_boxes[$this->_cl]->h;
