...
class Reports_model extends CI_Model{
	...
	public function daily_sales(){
		$this->db->select("
			...
			A.total_amount
		",false);
		$where = "DATE_FORMAT(A.dDate,  '%Y-%m-%d') between ".$this->db->escape($this->input->post('cFrom'))." and ".$this->db->escape($this->input->post('cTo'))." and A.InActive = 0 and B.InActive = 0";
		$this->db->where($where);	
	...
	public function patient_list(){
		...
		$where = "A.InActive = 0 and DATE_FORMAT( date_entry,  '%Y-%m-%d' ) between ".$this->db->escape($this->input->post('cFrom'))." and ".$this->db->escape($this->input->post('cTo'));
		if($this->input->post('gender') != ''){ 			$where .= " and C.param_id = ".$this->db->escape($this->input->post('gender'));}
		if($this->input->post('civil_status') != ''){ 		$where .= " and D.param_id = ".$this->db->escape($this->input->post('civil_status'));}
		if($this->input->post('bloodGroup') != ''){ 		$where .= " and E.param_id = ".$this->db->escape($this->input->post('bloodGroup'));}
		if($this->input->post('insurance_comp') != ''){ 	$where .= " and F.in_com_id = ".$this->db->escape($this->input->post('insurance_comp'));}
		$this->db->where($where);
	...
	public function doctor_fee_report(){
		...
		$where = "DATE_FORMAT( A.date,  '%Y-%m-%d' ) BETWEEN ".$this->db->escape($this->input->post('cFrom'))." AND ".$this->db->escape($this->input->post('cTo'))." 
				AND A.user_id LIKE '%{$doc}%' ESCAPE '!' AND C.receipt_no != ''";
		$this->db->where($where);
	...
	public function outpatient(){
		...
		$where = "A.InActive = 0 
				and A.date_visit between ".$this->db->escape($this->input->post('cFrom'))." and ".$this->db->escape($this->input->post('cTo'))." 
				and A.patient_type = 'OPD' and A.nStatus = 'Pending'";
		if($this->input->post('department') != ''){ $where .= " and H.department_id = ".$this->db->escape($this->input->post('department')); }
		$this->db->where($where);
	...
	public function inpatient(){
		...
		$where = "A.department_id LIKE '%{$dept}%' ESCAPE '!' 
	...
	public function ORList(){
		$this->db->select("
			...
			A.total_amount
		",false);
		$where = "DATE_FORMAT(A.dDate,  '%Y-%m-%d') between ".$this->db->escape($this->input->post('cFrom2'))." and ".$this->db->escape($this->input->post('cTo2'))." 
