...
class Appointment_model extends CI_Model{
	...
	/**
	 * Get escaped search term to prevent SQL injection
	 */
	private function _get_safe_search(){
		return $this->db->escape_like_str($this->session->userdata("search_patient_master"));
	}
	...
	public function getAll($limit = 10, $offset = 0){
		...
		$this->db->select("
			...
			A.date_entry
		",false);
		$search = $this->_get_safe_search();
		$where = "(
				A.lastname like '%".$search."%' or 
				A.firstname like '%".$search."%' or 
				A.middlename like '%".$search."%' or 
				C.cValue like '%".$search."%' or 
				D.cValue like '%".$search."%'
				) 
				and A.InActive = 0";
		$this->db->where($where);
