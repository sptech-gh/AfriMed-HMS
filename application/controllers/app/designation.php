...
class Designation extends General{
	...
	public function designation($offset = 0){
		...
		{	
				$deleteForm = '<form method="post" action="'.base_url().'app/designation/delete/'.$designation->designation_id.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to delete?\')">'
	...
	public function edit($id = 0){
		if(isset($_POST['btnSubmit'])){
			...
			if (empty($id) || !is_numeric($id)) {
				$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Invalid designation selected.</div>");
				redirect(base_url().'app/designation',$this->data);
				return;
			}
			// user restriction function
			...
			if (empty($this->data['designation'])) {
				$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Designation not found.</div>");
				redirect(base_url().'app/designation',$this->data);
				return;
			}
	...
	public function edit_save(){
		...
		if (empty($posted_id) || !is_numeric($posted_id)) {
			$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Invalid designation selected.</div>");
			redirect(base_url().'app/designation');
			return;
		}
		$record = $this->designation_model->getDesignation($posted_id);
		if (empty($record)) {
			$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Designation not found.</div>");
			redirect(base_url().'app/designation');
			return;
