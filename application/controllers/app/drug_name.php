...
class Drug_name extends General{
	...
	public function drug_master($offset = 0){
		...
		{	
				...
				$deleteForm = '<form method="post" action="'.base_url().'app/drug_name/delete/'.$drug_name->drug_id.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to delete?\')">'
	...
	public function add(){
		...
		$this->data['drug_forms'] = $this->drug_name_model->getDrugForms();
		$this->data['default_uom'] = $this->drug_name_model->getDefaultUOM();
	...
	public function edit($id = 0){
		if($this->input->method(TRUE) === 'POST'){
			...
			if (!$this->data['drug_name']) {
				$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Please choose a drug to edit.</div>");
				redirect(base_url().'app/drug_name',$this->data);
				return;
			}
	...
	public function edit_save(){
		...
		if ($id <= 0 || !$this->drug_name_model->getDrugName($id)) {
			$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Please choose a valid drug to edit.</div>");
			redirect(base_url().'app/drug_name',$this->data);
			return;
		}
