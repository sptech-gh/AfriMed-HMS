...
class Bill_group_name extends General{
	...
	public function group_name($offset = 0){
		...
		{	
				$deleteForm = '<form method="post" action="'.base_url().'app/bill_group_name/delete/'.$group_name->group_id.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to delete?\')">'
	...
	public function edit($id = 0){
		...
		}
		{
			$id = (int)$id;
			if ($id <= 0) {
				$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Invalid group name record.</div>");
				redirect(base_url().'app/bill_group_name');
				return;
			}
			// user restriction function
			...
			if (!$this->data['group_name']) {
				$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Group name record not found.</div>");
				redirect(base_url().'app/bill_group_name');
				return;
			}
