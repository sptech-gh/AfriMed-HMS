...
class User extends General{
	...
	public function __construct(){
		...
		// Allow users to access upload_picture and upload_na for their own profile
		$method = $this->router->fetch_method();
		$allowed_self_methods = array('upload_picture', 'upload_na');
		if (!in_array($method, $allowed_self_methods)) {
			require_role('admin');
		}
	}
	...
	public function user($offset = 0){
		...
		{	
				if($user->InActive == 1){
					...
					$InActive = '<form method="post" action="'.base_url().'app/user/activate/'.$user->user_id.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to Activate this user?\')">'
						.'<input type="hidden" name="'.$this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'">'
						.'<button type="submit" class="delete btn btn-xs btn-success">Activate</button>'
						.'</form>';
				}else{
					$cStatus = "Active";
					$InActive = '<form method="post" action="'.base_url().'app/user/delete/'.$user->user_id.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to InActive this user?\')">'
						.'<input type="hidden" name="'.$this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'">'
						.'<button type="submit" class="delete btn btn-xs btn-danger">InActive</button>'
						.'</form>';
				}
	...
	public function upload_picture($id){
		// Allow users to upload their own picture (bypass admin check for own profile)
		$current_user_id = $this->session->userdata('user_id');
		if ($id != $current_user_id && !has_role('admin')) {
			redirect(base_url().'access_denied');
			return;
		}
	...
	public function upload_na(){
		// Allow users to upload their own picture
		$upload_user_id = $this->input->post('user_id');
		$current_user_id = $this->session->userdata('user_id');
		if ($upload_user_id != $current_user_id && !has_role('admin')) {
			redirect(base_url().'access_denied');
			return;
		}
	...
	public function save(){
		// Only username, password, phone, email are compulsory
		$this->form_validation->set_rules("lastname","Last Name","trim|xss_clean|callback_validate_name");	
		...
		$this->form_validation->set_rules("user_role","User Role","trim|xss_clean|required");
		$this->form_validation->set_rules("phone","Phone No","trim|xss_clean|required");
	...
	public function edit($id = 0){
		if(isset($_POST['btnSubmit'])){
			...
			if (empty($id) || !is_numeric($id)) {
				$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Invalid user selected.</div>");
				redirect(base_url().'app/user',$this->data);
				return;
			}
			// user restriction function
			...
			if (empty($this->data['user'])) {
				$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>User not found.</div>");
				redirect(base_url().'app/user',$this->data);
				return;
			}
	...
	function edit_user(){
		...
		$record = $this->user_model->getUser($posted_id);
		if (empty($record)) {
			$this->session->set_flashdata('message',"<div class='alert alert-warning alert-dismissable'><i class='fa fa-warning'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>User not found.</div>");
			redirect(base_url().'app/user',$this->data);
			return;
		}
		// Only username, phone, email are compulsory (password only required if changing)
		$this->form_validation->set_rules("lastname","Last Name","trim|xss_clean|callback_validate_name_edit");	
		$this->form_validation->set_rules("firstname","First Name","trim|xss_clean");	
		$this->form_validation->set_rules("middlename","Middle Name","trim|xss_clean");
		$this->form_validation->set_rules("birthday","Birthday","trim|xss_clean");
		$this->form_validation->set_rules("phone","Phone No","trim|xss_clean|required");
