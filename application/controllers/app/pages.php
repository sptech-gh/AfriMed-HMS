...
class Pages extends General{
	...
	public function pages($offset = 0){
		...
		{	
				$deleteForm = '<form method="post" action="'.base_url().'app/pages/delete/'.$pages->page_id.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to delete?\')">'
					.'<input type="hidden" name="'.$this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'">'
					.'<input type="hidden" name="page_name" value="'.htmlspecialchars((string)$pages->page_name, ENT_QUOTES, 'UTF-8').'">'
