...
class Appointment extends General{
	...
	public function __construct(){
		...
		General::variable();
		require_role(array('admin', 'doctor', 'nurse', 'cashier', 'receptionist'));
	...
	public function index(){
		...
		if(General::has_rights_to_access($page_id->page_id,$userRole->user_role) == FALSE){
			if (!has_role(array('admin', 'doctor', 'nurse', 'cashier', 'receptionist'))) {
	...
	public function patient_master($offset = 0){
		...
		{	
			...
			{
				$checkInForm = '<form method="post" action="'.base_url().'app/appointment/checkInApp/'.$patient->appID.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to Check IN this Patient?\')">'
					.'<input type="hidden" name="'.$this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'">'
					.'<button type="submit" class="delete btn btn-xs btn-info">Check IN</button>'
					.'</form>';
			...
			if($patient->appointmentStatus == "A"){
				...
				$deleteForm = '<form method="post" action="'.base_url().'app/appointment/delete/'.$patient->appID.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to delete?\')">'
					.'<input type="hidden" name="'.$this->security->get_csrf_token_name().'" value="'.$this->security->get_csrf_hash().'">'
	...
	public function save(){
		...
		$this->form_validation->set_rules("mode","Mode","trim|xss_clean");
		$this->form_validation->set_rules("title","Title","trim|xss_clean|required");
		$this->form_validation->set_rules("lastname","Surname","trim|xss_clean|required");
		...
		$this->form_validation->set_rules("appAMPM","AM/PM","trim|xss_clean|required");
		$this->form_validation->set_rules("consultantDoctor","Consultant Doctor","trim|xss_clean|required");
		$this->form_validation->set_rules("appointmentReason","Appointment Reason","trim|xss_clean");
	...
	public function editAppointmentSave(){
		...
		if($this->form_validation->run()){
			...
			$this->session->set_flashdata('message',"<div class='alert alert-danger alert-dismissable'><i class='fa fa-exclamation-circle'></i><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><strong>Update failed.</strong> Please correct the errors and try again.</div>");
