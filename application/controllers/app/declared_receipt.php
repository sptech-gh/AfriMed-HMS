...
class Declared_receipt extends General{
	...
	public function index(){
		if(isset($_POST['btnView'])){
			...
			$this->audit_report_generation('financial', 'declared_receipt_daily_sales', 'browser');
	...
	public function receiptList($offset = 0){
		$this->require_report_scope('financial');
		$this->audit_report_generation('financial', 'declared_receipt_list', 'browser');
		$uri_segment = 4;
		...
		{		
				$deleteBtn = '<form method="post" action="'.base_url().'app/declared_receipt/delete/'.$roles->id.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to delete?\')">'
	...
	public function view_receipt_print($or_no){
		$this->require_report_scope('financial');
		$this->audit_report_generation('financial', 'declared_receipt_print', 'browser');
		$this->data['OR_details'] = $this->reports_model->OR_details($or_no);
		if ($this->data['OR_details'] && isset($this->data['OR_details']->invoice_no) && trim((string)$this->data['OR_details']->invoice_no) !== '') {
			$this->load->model('app/billing_model');
			$invNo = (string)$this->data['OR_details']->invoice_no;
			$rcpNo = isset($this->data['OR_details']->receipt_no) ? (string)$this->data['OR_details']->receipt_no : null;
			$payload = $this->billing_model->build_receipt_print_payload($invNo, $rcpNo);
			$okMatch = false;
			if (isset($payload['getOR']) && $payload['getOR'] && isset($payload['getOR']->receipt_no) && $rcpNo !== null) {
				$okMatch = (trim((string)$payload['getOR']->receipt_no) === trim((string)$rcpNo));
			}
			if ($okMatch) {
	...
	public function delete($id){
		...
		$this->db->query("UPDATE declaredor SET InActive = 1 WHERE id = ?", array((int)$id));
