...
class Surgical_package extends General{
	...
	public function surgical_package($offset = 0){
		...
		{	
				$deleteForm = '<form method="post" action="'.base_url().'app/surgical_package/delete/'.$department->surgery_id.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to delete?\')">'
	...
	public function save_item(){
		...
		if($this->form_validation->run()){
			...
			$this->db->query("UPDATE surgical_package SET total_costs = 
				(SELECT SUM(costs) FROM surgical_package_t WHERE surgery_id = ? AND InActive = '0')
				WHERE surgery_id = ?", array((int)$this->input->post('surgery_id'), (int)$this->input->post('surgery_id')));
	...
	public function delete_item(){
		...
		$this->db->query("UPDATE surgical_package_t SET InActive = '1' WHERE m_id = ?", array((int)$m_id));	
		
		$this->db->query("UPDATE surgical_package SET total_costs = 
					(SELECT SUM(costs) FROM surgical_package_t WHERE surgery_id = ? AND InActive = '0')
					WHERE surgery_id = ?", array((int)$surgery_id, (int)$surgery_id));
