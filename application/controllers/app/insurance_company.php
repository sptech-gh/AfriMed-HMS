...
class Insurance_company extends General{
	...
	public function __construct(){
		...
		General::variable();
		require_role('admin');
		if (!$this->session->userdata('_schema_insurance_ok')) {
			$this->insurance_company_model->ensure_insurance_schema();
			$this->session->set_userdata('_schema_insurance_ok', 1);
		}
	}
	...
	public function insurance($offset = 0){
		...
		$this->table->set_heading('Company Name', 'Insurance Type', 'Billing Type', 'Email Address','Action');
		$i = 0 + $offset;
		
		
		foreach ($insurance as $insurance)
		{	
				$ins_type = isset($insurance->insurance_type) ? $insurance->insurance_type : '';
				$bill_type = isset($insurance->billing_type) ? $insurance->billing_type : '';
				$type_label = '<span class="label label-'.($ins_type === 'NHIS' ? 'success' : ($ins_type === 'NONE' ? 'default' : ($ins_type === 'CORPORATE' ? 'warning' : 'info'))).'">'.$ins_type.'</span>';
				$bill_label = '<span class="label label-'.($bill_type === 'CASH' ? 'default' : ($bill_type === 'NHIS' ? 'success' : 'info')).'">'.$bill_type.'</span>';
				$deleteForm = '<form method="post" action="'.base_url().'app/insurance_company/delete/'.$insurance->in_com_id.'" style="display:inline;" onsubmit="return confirm(\'Are you sure want to delete?\')">'
